...
| Worker Pub/Sub não inicia | Verificar `PUBSUB_ENABLED=true`, `GOOGLE_APPLICATION_CREDENTIALS` acessível, tópico/subscrição existem, e service account tem `roles/pubsub.subscriber` + `roles/pubsub.publisher` |

---

## 21. Transporte Pub/Sub (Worker Mode)

O Agent Bot pode rodar em **dois modos** a partir do mesmo código:

| Modo | Comando | Quando usar |
|------|---------|-------------|
| HTTP | `uvicorn app.main:app --port 8002` | Testes diretos, curl, Postman, integrações síncronas |
| Pub/Sub Worker | `python -m app.worker_main` | Google Chat, integrações assíncronas, produção |

Os dois modos compartilham 100% do `app/agent/orchestrator.py` — o worker apenas envelopa o `process_message(ChatRequest)` em um handler de mensagem Pub/Sub.

### 21.1 Arquitetura

```
Pub/Sub "incoming" subscription
    │  streaming pull (concurrency=4)
    ▼
[AgentSubscriber] ──► handle_incoming(message, publisher)
                        │
                        ├─► Redis dedup (request_id, TTL 1h)
                        │
                        ├─► process_message(ChatRequest)   ◄── mesmo do HTTP
                        │     ├─► Router (conversa_comum | pims)
                        │     ├─► RAG (Qdrant)
                        │     ├─► Sub-agent (ADK + MCP ou LiteLLM)
                        │     └─► Memory (Redis)
                        │
                        ├─► Build OutgoingMessage
                        │
                        ├─► Publish to "outgoing" topic    ──► [external bridge]
                        │
                        └─► Ack Pub/Sub message
```

### 21.2 Pacote `app/pubsub/`

| Arquivo | Função |
|---------|--------|
| `schemas.py` | `IncomingMessage`, `OutgoingMessage` (Pydantic) |
| `publisher.py` | `AgentPublisher` — wrapper async de `PublisherClient` |
| `subscriber.py` | `AgentSubscriber` — streaming pull com `FlowControl(max_messages=4)` |
| `worker.py` | `handle_incoming()` — parse, dedup, process, publish, ack/nack |
| `runner.py` | `run_pubsub_worker()` — lifecycle, signal handling, graceful shutdown |
| `../worker_main.py` | Entry point: `python -m app.worker_main` |

### 21.3 Contrato de Mensagens

**Incoming** (publicado no tópico de entrada pelo bridge externo):
```json
{
  "request_id": "uuid",
  "user_id": "u-123",
  "conversation_id": "c-456",
  "message": "qual o valor da tag X?",
  "images": []
}
```

**Outgoing** (publicado no tópico de saída pelo agente):
```json
{
  "request_id": "uuid",
  "type": "response | error",
  "content": "O valor da tag X é 12.5 °C.",
  "categoria": "pims",
  "next_action": "pi_agent",
  "has_image": false,
  "tags_encontradas": [],
  "agent_trace": [],
  "error": null,
  "timestamp": "2026-06-16T14:23:00-03:00"
}
```

Atributos Pub/Sub também são populados: `request_id`, `type`.

### 21.4 Variáveis de Ambiente

| Variável | Padrão | Descrição |
|----------|--------|-----------|
| `PUBSUB_ENABLED` | `false` | Liga o modo worker |
| `PUBSUB_PROJECT_ID` | `None` | GCP project id |
| `PUBSUB_INCOMING_SUBSCRIPTION` | placeholder | Path completo: `projects/<id>/subscriptions/<name>` |
| `PUBSUB_OUTGOING_TOPIC` | placeholder | Path completo: `projects/<id>/topics/<name>` |
| `PUBSUB_WORKER_CONCURRENCY` | `4` | `FlowControl.max_messages` (in-flight) |
| `PUBSUB_DEDUP_TTL_SECONDS` | `3600` | TTL dos markers de dedup em Redis |
| `PUBSUB_SHUTDOWN_TIMEOUT_SECONDS` | `30` | Espera por handlers em-flight no SIGTERM |
| `GOOGLE_APPLICATION_CREDENTIALS` | `None` | Path para service account JSON |

### 21.5 Permissões GCP Necessárias

A service account precisa de:
- `roles/pubsub.subscriber` na subscription
- `roles/pubsub.publisher` no topic
- `roles/pubsub.viewer` no projeto (opcional, para debug)

### 21.6 Setup Inicial (referência)

Caso os recursos ainda não existam:

```bash
# 1. Criar tópico de saída
gcloud pubsub topics create agent-bot-outgoing \
  --project=your-gcp-project

# 2. Criar subscription no tópico de entrada
gcloud pubsub subscriptions create agent-bot-incoming-sub \
  --project=your-gcp-project \
  --topic=agent-bot-incoming \
  --ack-deadline=120 \
  --message-retention-duration=7d

# 3. Service account
gcloud iam service-accounts create agent-bot-pubsub \
  --project=your-gcp-project

gcloud pubsub subscriptions add-iam-policy-binding agent-bot-incoming-sub \
  --project=your-gcp-project \
  --member="serviceAccount:agent-bot-pubsub@your-gcp-project.iam.gserviceaccount.com" \
  --role="roles/pubsub.subscriber"

gcloud pubsub topics add-iam-policy-binding agent-bot-outgoing \
  --project=your-gcp-project \
  --member="serviceAccount:agent-bot-pubsub@your-gcp-project.iam.gserviceaccount.com" \
  --role="roles/pubsub.publisher"

gcloud iam service-accounts keys create gcp-key.json \
  --iam-account=agent-bot-pubsub@your-gcp-project.iam.gserviceaccount.com
```
...
**Docker Compose:**
```bash
# Criar ./secrets/gcp-key.json com a chave da service account
docker compose up -d worker
```

### 21.8 Idempotência

Pub/Sub garante **at-least-once delivery**, então mensagens podem ser duplicadas. O worker usa Redis (`pubsub:processed:<request_id>`, TTL 1h) para deduplicação:

1. Ao receber uma mensagem, verifica se `request_id` já foi processado.
2. Se sim → `ack` imediato, sem chamar o LLM.
3. Se não → processa normalmente e marca como processado **após** publicar o `OutgoingMessage`.

Se a publicação falhar, a marcação é pulada e a mensagem é `nack`-ada para retry.

### 21.9 Graceful Shutdown

Em SIGTERM/SIGINT:
1. `subscriber.stop()` cancela o streaming pull.
2. Aguarda os handlers em-flight terminarem (timeout configurável).
3. `publisher.close()` libera o gRPC channel.
4. Exit code 0.

Handlers que excedem o timeout são cancelados, e suas mensagens Pub/Sub voltam para a fila (serão reprocessadas na próxima inicialização graças à dedup).
